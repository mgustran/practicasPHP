...
<div id="page">
    ...
    if (isset($_GET['action']) == 'perm') {
        ...
        <form action="perms.php" method="post">
        	<label for="permName">Name:</label><input type="text" name="permName" id="permName" value="<?php  $myACL->getPermNameFromID(empty($_GET['permID'])); ?>" maxlength="30" /><br />
            <label for="permKey">Key:</label><input type="text" name="permKey" id="permKey" value="<?php  $myACL->getPermKeyFromID(empty($_GET['permID'])); ?>" maxlength="30" /><br />
    	<input type="hidden" name="action" value="savePerm" />
        <input type="hidden" name="permID" value="<?php echo empty($_GET['permID']); ?>" />
    	<input type="submit" name="Submit" value="Submit" />
    </form>
    <form action="perms.php" method="post">
         <input type="hidden" name="action" value="delPerm" />
         <input type="hidden" name="permID" value="<?php echo isset($_GET['permID'])? $_GET['permID'] : empty($_GET['permID']); ?>" />
